...
def load_dataset_ed(fname):
  ...
  dialogue = {}
  utt = df['Utterance'].tolist()  # load the list of utterances
  dia_id = df['Dialogue_ID'].tolist()  # load the list of dialogue id's
  utt_id = df['Utterance_ID'].tolist()  # load the list of utterance id's
  emotions = df['Emotion'].tolist()  # load the list of emotions
  speakers = df['Speaker'].tolist()  # load the list of emotions
...
def main(
  ...
  result_file = f"meld/results/si/{model_name}_{data_nums}s_{int(time.time())}.json"
